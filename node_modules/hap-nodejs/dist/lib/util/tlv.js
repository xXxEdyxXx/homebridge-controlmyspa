    ...
    for (; leftBytes > 0;) {
        ...
        if (existing) { // there is already an item with the same type
            if (lastItemWasDelimiter && lastType === type) { // list of tlv types
                ...
                throw new Error("Found duplicated tlv entry with type ".concat(type, " and length ").concat(length, " ")
                    + "(lastItemWasDelimiter: ".concat(lastItemWasDelimiter, ", lastType: ").concat(lastType, ", lastLength: ").concat(lastLength, ")"));
            }
        }
        else {
            result[type] = data;
