...
var HAPConnection = /** @class */ (function (_super) {
    ...
    HAPConnection.prototype.getLocalAddress = function (ipVersion) {
        var e_6, _a;
        var interfaceDetails = os_1.default.networkInterfaces()[this.networkInterface];
        if (!interfaceDetails) {
            throw new Error("Could not find " + ipVersion + " address for interface " + this.networkInterface);
        }
        // Find our first local IPv4 address.
        if (ipVersion === "ipv4") {
            var ipv4Info = interfaceDetails.find(function (info) { return info.family === "IPv4"; });
            if (ipv4Info) {
                return ipv4Info.address;
            }
            throw new Error("Could not find " + ipVersion + " address for interface " + this.networkInterface + ".");
        }
        var localUniqueAddress;
        try {
            for (var _b = tslib_1.__values(interfaceDetails.filter(function (entry) { return entry.family === "IPv6"; })), _c = _b.next(); !_c.done; _c = _b.next()) {
                var v6entry = _c.value;
                if (!v6entry.scopeid) {
    ...
    HAPConnection.getLocalNetworkInterface = function (socket) {
        ...
        var defaultInterface = function () { var _a, _b; return (_b = (_a = Object.entries(interfaces).find(function (_a) {
            var _b = tslib_1.__read(_a, 2), addresses = _b[1];
            return addresses === null || addresses === void 0 ? void 0 : addresses.some(function (address) { return !address.internal; });
        })) === null || _a === void 0 ? void 0 : _a[0]) !== null && _b !== void 0 ? _b : "unknown"; };
        ...
        try {
            ...
            for (var _c = tslib_1.__values(Object.entries(interfaces)), _d = _c.next(); !_d.done; _d = _c.next()) {
                var _e = tslib_1.__read(_d.value, 2), name = _e[0], addresses = _e[1];
                if (addresses === null || addresses === void 0 ? void 0 : addresses.some(function (_a) {
        ...
        try {
            ...
            for (var _f = tslib_1.__values(Object.entries(interfaces)), _g = _f.next(); !_g.done; _g = _f.next()) {
                var _h = tslib_1.__read(_g.value, 2), name = _h[0], addresses = _h[1];
                if (addresses === null || addresses === void 0 ? void 0 : addresses.some(function (entry) { return entry.family === family && (0, domain_formatter_1.getNetAddress)(localAddress, entry.netmask) === (0, domain_formatter_1.getNetAddress)(entry.address, entry.netmask); })) {
                    return name;
                }
            }
        }
