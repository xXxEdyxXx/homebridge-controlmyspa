...
function DBusBuffer(buffer, startPos, options) {
  ...
  this.buffer = buffer;
  // littleEndian defaults to true; message.js sets it from the message header byte.
  this.le = options.littleEndian !== false;
...
DBusBuffer.prototype.readSimpleType = function readSimpleType(t) {
  ...
  switch (t) {
    ...
    case 'x':
      //signed
      this.align(3);
      // In LE the low 32 bits come first; in BE the high 32 bits come first.
      // Long.fromBits(low, high, unsigned).
      word0 = this.readInt32();
      word1 = this.readInt32();
      data = this.le
        ? Long.fromBits(word0, word1, false)
        : Long.fromBits(word1, word0, false);
    ...
    case 't':
      ...
      word1 = this.readInt32();
      data = this.le
        ? Long.fromBits(word0, word1, true)
        : Long.fromBits(word1, word0, true);
      if (this.options.ReturnLongjs) return data;
      return data.toNumber(); // convert to number (good up to 53 bits)
    case 'd':
