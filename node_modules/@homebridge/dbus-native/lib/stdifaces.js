  ...
  if (
    ...
    for (var path in bus.exportedObjects) {
      if (path.indexOf(msg.path) === 0) {
        ...
        if (path === msg.path) {
          nodes[path] = bus.exportedObjects[path];
        } else {
    ...
    if (msg.member === 'Get' || msg.member === 'Set') {
      ...
      if (msg.member === 'Get') {
        ...
        // msg.body[2] is the variant: [parsedSignatureTree, [value]]
        var variant = msg.body[2];
        if (
          ...
          bus.sendError(
            ...
            'Properties.Set: malformed variant value'
          );
          return 1;
        }
        impl[propertyName] = variant[1][0];
      }
